...
export class ManageGuideComponent implements OnInit {
    ...
    async add() {
        this.dialogService
            ...
            .onClose.subscribe(async (data: { fullName: string; email: string; phone: any; address: string; birthday: Date }) => {
                if (data) {
                    try {
                        console.log(data);
                        let i = await this.guideService.addGuide(data.fullName, data.email, data.address, data.phone);
