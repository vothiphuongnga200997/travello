...
export class ManageGuideComponent implements OnInit {
    ...
    async add() {
        this.dialogService
            ...
            .onClose.subscribe(async (data: { fullName: string; email: string; phone: number; address: string; birthday: Date }) => {
                if (data) {
                    try {
                        console.log(data);
                        let i = await this.guideService.addGuide(data.fullName, data.email, data.phone, data.address, data.birthday);
