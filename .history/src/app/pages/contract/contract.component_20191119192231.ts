...
export class MoneyComponent implements ViewCell, OnInit {
    renderValue: string;
    ...
    settings = {
        ...
        columns: {
            ...
            money: {
                title: 'Thanh toán',
                type: 'custom',
                renderComponent: MoneyComponent,
            },
            createAt: {
                title: 'Ngày đặt',
                type: 'string',
    ...
    async getContract() {
        ...
        if (result) {
            for (let data of result) {
                ...
                let t = [];
                t.push({
                    price: this.formatNumber(data.get('price')),
                    paid: this.formatNumber(data.get('paid')),
                    indemnification: data.get('indemnification'),
                    expiryDate: data.get('expiryDate'),
                });
                'Tổng tiền: ' + +' Đã trả:' + +' Còn lại' + this.formatNumber(data.get('price') + +data.get('paid'));
                this.contract.push({
                    id: data.id,
