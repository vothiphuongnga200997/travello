...
export class AddCustomerComponent implements OnInit {
    ...
    async pay() {
        ...
        if (data.tour !== '' && data.contact.birthday !== '') {
            for (let i of this.form.value.contacts) {
                let secondDate = moment(i.birthdat).format('YYYY/MM/DD');
                let diffInDays = Math.abs(firstDate.diff(secondDate, 'year'));
                if (diffInDays > 5) this.price += this.form.value.tour.adulPrice;
                else {
                    this.price += this.form.value.tour.childrenPrice;
                }
