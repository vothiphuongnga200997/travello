...
export class AddCustomerComponent implements OnInit {
    ...
    idCustomer: '';
    priceSurcharge: number = 0; // so tien theo danh sach dc chon
    listSurcharge: Array<any>; // danh sach dc chon > 0
    priceSA: number = 0;
    priceSC: number = 0;
    ...
    constructor(
        ...
        this.getTour();
        this.dataService = completerService.local(this.objUser, 'email', 'email');
    }
    ...
    // form
    async getTour() {
        let result = await this.contractService.getScheduleAdd();
        for (let i of result) {
            this.schedule.push({
                id: i.id,
    ...
    selectedTour(i) {
        this.price = 0;
        this.sumMoney = 0;
        this.startDay = this.schedule[i].startDay;
        this.endDay = this.schedule[i].endDay;
        ...
        this.priceSA = this.schedule[i].priceSA;
        this.priceSC = this.schedule[i].priceSC;
    ...
    async pay() {
        ...
        this.price = this.numberAdult * this.priceSA + this.numberKids * this.priceSC;
        this.sumMoney =
            this.priceSurcharge +
    ...
    async submit() {
        ...
        if (this.form.invalid) {
            ...
            this.ref.close({
                ...
                listSurcharge: this.listSurcharge,
