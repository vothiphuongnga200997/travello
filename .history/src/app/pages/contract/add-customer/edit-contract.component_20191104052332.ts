...
export class EditContractComponent implements OnInit {
    ...
    async ngOnInit() {
        ...
        console.log(result);
        if (result) {
            let schedule = await this.tourService.querySchedule(result.get('objSchedule'));
        ...
        this.form1 = this.fb.group({
            ...
            tour: [result.attributes.objTour.id],
            adult: [result.attributes.numberAdult],
            kids: [result.attributes.numberKids],
            paid: [result.attributes.paid],
            contacts: this.fb.array([]),
        });
        this.startDay = moment(result.attributes.objTour.attributes.startDay).format('HH:mm DD/MM/YYYY');
        this.endDay = moment(result.attributes.objTour.attributes.endDay).format('HH:mm DD/MM/YYYY');
        this.adultPrice = result.attributes.objTour.attributes.adultPrice;
        this.childrenPrice = result.attributes.objTour.attributes.childrenPrice;
        this.saleoff = result.attributes.objTour.attributes.saleoff;
        this.empty = result.attributes.objTour.attributes.empty;
