...
@Component({
    ...
    template: `
        <div style="width:180px">
            <a class="mb-1"
                >Tổng tiền: <a class="font-weight-bold"> {{ this.value[0].sumMoney | number }} </a>
            </a>
            <p class="mb-1">Đã thanh toán: {{ this.value[0].paid | number }}</p>
            <p class="mb-1">Còn lại: {{ this.value[0].unpaid | number }}</p>
            <p class="mb-1" *ngIf="this.date" class="text-danger">{{ this.date | number }}</p>
        </div>
    `,
})
