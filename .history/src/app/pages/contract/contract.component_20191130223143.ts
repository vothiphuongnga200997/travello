...
@Component({
    ...
    template: `
        <div style="width:180px">
            ...
            <a class="mb-1">Đã thanh toán:  <a class="font-weight-bold">  {{ this.value[0].paid | number }} </a></a>
            <a class="mb-1">Còn lại:  <a class="font-weight-bold">  {{ this.value[0].unpaid | number }}  <a class="font-weight-bold"> </a>
            <a class="mb-1" *ngIf="this.date" class="text-danger">{{ this.date | number }}</a>
        </div>
    `,
})
export class PayComponent implements ViewCell, OnInit {
    ...
    ngOnInit() {
        ...
        if (
            ...
            this.date = 'Hạn thanh toán' + moment(this.value[0].expiry).format('HH:mm DD/MM/YYYY');
        else {
            this.date = '';
        }
    }
    ...
    async getContract() {
        ...
        if (result) {
            for (let data of result) {
                ...
                queryCustomer.equalTo('objUser', data.get('objUser'));
                let resultCustomer = await queryCustomer.first();
                sumMoney = data.get('price') + moneySurcharge;
                if (data.get('objSchedule').get('startDay') < date) {
                    sumMoney = data.get('price') + moneySurcharge;
                } else {
                    if (resultCustomer) {
                        sumMoney = data.get('price') - data.get('price') * resultCustomer.get('discount') + moneySurcharge;
                    } else {
                        sumMoney = data.get('price') + moneySurcharge;
                    }
                }
                ...
                this.contract.push({
                    ...
                    price: data.get('price') - data.get('price') * resultCustomer.get('discount') + moneySurcharge,
                    tourQuantity: resultShedule.get('objTour').get('quantity'),
                    status: this.status,
                    idUser: data.get('objUser'),
                    paid: data.get('paid'),
                    paidOfCustomer: resultCustomer.get('paid'),
