...
@Component({
    ...
    template: `
        <nb-card>
            ...
            <nb-card-body>
                <div>Bạn muốn xóa hợp đồng {{ this.event.data.data.id }}</div>
                <div class="footer">
...
export class DeleteComponent implements OnInit {
    ...
    async delete() {
        let startDate = moment(this.event.data.data.startDay, 'DD/MM/YYYY');
        ...
        if (diffInDays < 7 && diffInDays >= 0) {
            ...
            query.equalTo('objectId', this.event.data.data.id);
            let dataSave: any = {};
            let dataTour: any = {};
            this.event.data.data.tourist.createAt = new Date();
            dataSave.objectId = this.event.data.data.id;
            dataSave.infoCustom = [];
            dataSave.cancelContract = this.event.data.data.tourist;
            dataSave.numberAdult = 0;
            dataSave.numberKids = 0;
            dataSave.indemnification = this.event.data.data.price / 2;

            try {
                // let result = await obj.save(dataSave);
                // if (result) {
                //     dataTour.empty = await this.contractService.setEmpty(this.event.data.data.idTour, this.event.data.data.quantity);
                //     dataTour.objectId = this.event.data.data.idTour;
            ...
            try {
                ...
                // if (deleteC) {
                //     dataTour.objectId = this.event.data.data.idTour;
                //     dataTour.empty = await this.contractService.setEmpty(this.event.data.idTour, this.event.data.quantity);
