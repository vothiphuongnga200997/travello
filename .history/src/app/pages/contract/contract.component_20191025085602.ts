...
export class ContractComponent implements OnInit {
    ...
    async getContract() {
        ...
        this.contract = [];
        if (result) {
            let dataSourses = result.map(async data => {
                let email = await this.contractService.getUserId(data.get('objTour').id);
                console.log(email);
                //    return {
                //         id: data.id,
                //         fullname: data.get('objUser').get('fullname'),
                //         email: data.get('objUser').get('email'),
    ...
    deleteTicket(idContract, startDay, numberContract, numberTourist, info, idTour, quantity) {
        this.deleteArray = this.contract[numberContract].tourist.slice();
