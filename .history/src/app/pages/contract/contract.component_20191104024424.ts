...
@Component({
    ...
    template: `
        <div>
            <a class="d-flex justify-content-center text-success" *ngIf="value === 'pending'"> Mở </a>
            <a class="d-flex justify-content-center text-warning" *ngIf="value === 'done'"> Đóng </a>
            <a class="d-flex justify-content-center text-warning" *ngIf="value !== 'done' && value !== 'pending'"> {{ value }} </a>
        </div>
    `,
})
...
export class ContractComponent implements OnInit {
    ...
    async getContract() {
        ...
        if (result) {
            for (let data of result) {
                querySchedule.equalTo('objectId', data.get('objShedule').id);
                ...
                if (diffInDays > 3) {
                    this.status = `${diffInDays}`;
                }
                if (data.get('objSchedule').get('endDay') < dateNow) this.status = 'done';
                ...
                this.contract.push({
                    ...
                    idTour: data.get('objSchedule').id,
    ...
    async filter() {
        ...
        if (result) {
            for (let data of result) {
                let email = await this.contractService.getUserId(data.get('objUser').id);
                if (data.get('objTour').get('endDay') < dateNow) this.status = 'done';
                else this.status = 'pending';
                this.contract.push({
                    id: data.id,
                    fullname: data.get('objUser').get('fullname'),
