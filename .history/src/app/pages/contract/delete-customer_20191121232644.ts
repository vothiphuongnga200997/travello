    ...
    async delete() {
        ...
        if ((diffInDays < 7 && diffInDays >= 0) || this.event.status === 'done') {
            ...
            if (this.event.paid > 0) {
                ...
                console.log(result);
                if (result) {
                    let resulSchedule = await ObjectSchedule.save(dataSchedule);
                    if (resulSchedule) {
                        dataRepay.objectSchedule = ObjectSchedule.createWithoutData(resulSchedule.id);
                    }
                }
            } else {
                try {
                    ...
                    if (deleteC) {
                        dataSchedule.objectId = this.event.idTour;
                        dataSchedule.empty = await this.contractService.setEmpty(this.event.idSchedule, this.event.tourQuantity);
