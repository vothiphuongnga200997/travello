...
export class ContractComponent implements OnInit {
    ...
    @ViewChild('confirmDeleteDialog', { static: false }) deleteDialog: DialogComponent;
    ...
    async onDelete(event) {
        try {
            let result = await this.contractService.delete(event);
            if (result) {
                await this.getContract();
                this.toastrService.success(`Delete tour ${event} success`, `Delete tour`);
            } else {
                this.toastrService.error(`Delete tour ${event} fail`, `Delete tour`);
            }
        } catch (error) {
