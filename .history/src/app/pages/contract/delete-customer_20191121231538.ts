...
export class DeleteComponent implements OnInit {
    ...
    async delete() {
        ...
        if ((diffInDays < 7 && diffInDays >= 0) || this.event.status === 'done') {
            ...
            try {
                ...
                if (result) {
                    dataSchedule.empty = await this.contractService.setEmpty(this.event.idTour, this.event.tourQuantity);
            ...
            if (this.event.paid > 0) {
                ...
                let result = await contract.save(dataContract);
                console.log(result);
                if (result) {
                    dataRepay.objContract =  contract.createWithoutData(result.id);
            ...
            try {
                ...
                if (deleteC) {
                    dataTour.objectId = this.event.idTour;
                    dataTour.empty = await this.contractService.setEmpty(this.event.idTour, this.event.tourQuantity);
