...
export class SumMoneyComponent implements AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit() {
        this.themeSubscription = this.theme.getJsTheme().subscribe(async config => {
            ...
            const repay = Parse.Object.extend('tourCost');
            ...
            for (let i = 1; i <= 12; i++) {
                ...
                const queryRepay = new Parse.Query(repay);
                ...
                for (let data of result) {
                    queryRepay.equalTo('objSchedule', data);
                    let resultRepay = await queryRepay.first();
                    console.log(resultRepay);
                    let sum = 0;
                    if (resultRepay) {
                        for (let data1 of resultRepay.get('cost')) {
                            sum += data1.money;
                        }
                    }
                    this.chi[i - 1] = sum;
