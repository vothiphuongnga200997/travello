...
@Component({
    ...
    template: `
        <div style="height: 200px; background: white; float: right; margin:10px">
            <nb-select style="width: 50% margin: 10px" name="platForm" [(selected)]="commonSelectedItem">
                <nb-option (click)="chart()" *ngFor="let i of this.listYear" [value]="i">{{ i }}</nb-option>
            </nb-select>
        </div>
        <div echarts [options]="options" class="echart"></div>
        <div class="row m-2">
            <div class="col-1 bg-danger"></div>
            <div class="col-5">Số khách đã hủy</div>

            <div class="col-1 bg-primary"></div>
            <div class="col-5">Số khách đã đặt</div>
        </div>
    `,
})
