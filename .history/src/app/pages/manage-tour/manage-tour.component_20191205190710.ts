...
export class ManageTourComponent implements OnInit {
    ...
    async onDelete() {
        try {
            let result = await this.tourService.deleteTour(this.idDelete);
            if (result) {
                this.toastrService.success(`Delete tour success`, `Delete tour`);
                this.getTour();
            } else {
                this.toastrService.error(`Delete tour fail`, `Delete tour`);
            }
        } catch (error) {
