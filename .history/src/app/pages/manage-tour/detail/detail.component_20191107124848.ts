    ...
    template: `
        <nb-card style="height:500px ; width: 900px">
            <nb-card-body>
                ...
                <button type="button" class="btn-pages float-right"><i class="fas fa-plus-square"></i> Thêm hợp đồng</button>
                <br />
                ...
                ><br />
                <ng2-smart-table id="mytable" [settings]="settings" [source]="source"> </ng2-smart-table>
            </nb-card-body>
        </nb-card>
    `,
})
...
export class DetailComponent implements OnInit {
    ...
    async getSchedule() {
        ...
        for (let data of result) {
            if (data.get('startDay') > Date.now()) checkDay = 'Đã khởi hành';
            else {
                checkDay = 'Chưa khởi hành';
            }
    ...
    async getTourist(objSchedule) {
        this.windowService.open(ListTouristComponent, {
    ...
    async inputCost(objSchedule) {
        this.windowService.open(CostComponent, {
