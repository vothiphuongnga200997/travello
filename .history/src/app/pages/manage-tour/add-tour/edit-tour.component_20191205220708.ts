...
@Component({
    ...
    template: `
        ...
        <nb-card style="width: 350px;height: 180px;">
            ...
            <nb-card-body>
                <div *ngIf="!this.date">
                    <a>Tour này đã khởi hành không thể xóa</a>
                </div>
                <div *ngIf="this.date">
                    <div>Bạn muốn xóa ngày {{ this.startDay | date: 'short' }}</div>
                    <div class="footer">
...
export class DeleteComponent implements OnInit {
    ...
    schedule: any;
    date: boolean = true;
    constructor(protected ref: NbDialogRef<DeleteComponent>, protected tourService: TourService) {}
    ngOnInit() {
        if (this.endDay < new Date()) {
            this.date = false;
        }
        console.log('scheudle', this.schedule);
        console.log('objTour', this.objTour);
    }
    dismiss() {
        this.ref.close();
    }
    async delete() {
        let i = [];
        i.push(this.schedule);

        let result = await this.tourService.deleteTour(this.objTour, i);
        // this.ref.close({
        //     id: this.id,
        // });
    }
...
export class EditTourComponent implements OnInit {
        ...
        if (result) {
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                this.tableSchedule.push({
                    id: result[i].id,
                    obj: result[i],
                    startDay: moment(result[i].get('startDay')).format('DD/MM/YYYY, h:mm A'),
                    endDay: moment(result[i].get('endDay')).format('DD/MM/YYYY, h:mm A'),
                    hotel: result[i].get('hotel'),
                    guide: data,
                    codeSchedule: result[i].get('codeSchedule'),
                    end: result[i].get('endDay'),
                    tour: result[i].get('objTour'),
                });
            }
        }
        this.guideFirst = [];
    ...
    deleteSchedule(id, startDay, i, endDay) {
        this.dialogService
            .open(DeleteComponent, {
                context: {
                    objTour: this.tableSchedule[i].tour,
                    startDay: startDay,
                    endDay: endDay,
                    schedule: this.tableSchedule[i].obj,
                },
    ...
    async onSubmit() {
        ...
        if (this.listLocations.length > 0 && this.images.length > 0 && this.checkCode === '') {
            this.loadingService.start();
            this.obj = await this.tourService.editTour(this.registerForm.value, this.listLocations, this.locationFirst, this.images);
