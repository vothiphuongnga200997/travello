...
export class AddTourComponent implements OnInit {
    ...
    async checkDate() {
        ...
        if (this.startDay && this.endDay && this.numberRun !== 0) {
            this.checkDay.push({
                start: this.startDay,
                endDay: this.endDay,
            });
            for (let i = 1; i < this.checkDay.length; i++) {
                if (Date.parse(this.checkDay[i - 1].endDay) !== Date.parse(this.checkDay[i].endDay)) console.log('bang');
                else {
                    console.log(this.checkDay[i - 1].endDay);
                    console.log(this.checkDay[i].endDay);
                    this.getGuides = [];
                    this.listGuides = [];
                    let result = await this.guideService.findTour(this.startDay, this.endDay);
