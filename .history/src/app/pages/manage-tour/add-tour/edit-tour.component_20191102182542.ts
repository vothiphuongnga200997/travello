...
export class EditTourComponent implements OnInit {
    ...
    async ngOnInit() {
        this.registerForm = this.formBuilder.group({
            ...
            nameTour: [null, Validators.required],
            duration: [null, Validators.required],
            childrenPrice: [null, Validators.required],
            adultPrice: [null, Validators.required],
    ...
    async getForm() {
        if (this.obj.info.get('saleoff') > 0) {
            this.sale = true;
        }
        this.registerForm = this.formBuilder.group({
        ...
        let images = await this.tourService.getImage(this.obj.info);
        ...
        for (let i = 0; i < this.obj.schedule.length; i++) {
            ...
            this.tableSchedule.push({
                id: this.obj.schedule[i].id,
                startDay: this.obj.schedule[i].get('startDay'),
                endDay: this.obj.schedule[i].get('endDay'),
                hotel: this.obj.schedule[i].get('hotel'),
                guide: data,
            });
        }
    }
