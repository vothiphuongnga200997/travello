...
export class AddTourComponent implements OnInit {
    ...
    protected async onSelectedGuide(item: CompleterItem) {
        if (item) {
            let result = await this.guideService.findGuide(item.originalObject.id);
            if (result) {
                result.map(async res => {
                    let tour = await this.guideService.getTour(res);
                    if (tour) {
                        for (let i of tour) {
                            let startGuide = moment(i.get('startDay')).format('YYYY-MM-DD');
                            let endGuide = moment(i.get('endDay')).format('YYYY-MM-DD');
                            let endDay = moment(new Date(this.endDay)).format('YYYY-MM-DD');
                            let startDay = moment(new Date(this.startDay)).format('YYYY-MM-DD');
                            if (startGuide < endDay && endGuide > startDay) {
                                this.requiredGuide = await 'Trùng thời điểm';
                            } else {
                                this.requiredGuide = '';
                                this.guide.push({
                                    id: item.originalObject.id,
