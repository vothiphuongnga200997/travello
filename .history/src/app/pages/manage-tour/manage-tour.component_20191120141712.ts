...
export class ManageTourComponent implements OnInit {
    ...
    async getTour() {
        ...
        if (this.data) {
            ...
            this.infoTour.forEach(function(a) {
                groupByName[a.quantity] = groupByName[a.quantity] || [];
                groupByName[a.quantity].push({
                    obj: a.info,
                    code: a.info.get('code'),
                    name: a.info.get('nameTour'),
                    quantity: a.info.get('quantity'),
                    pending: pending,
                    sumSchedule: a.schedule.length,
                    saleoff: this.saleoff,
                });
            });
