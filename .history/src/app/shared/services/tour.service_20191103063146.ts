...
export class TourService {
    ...
    async editSchedule(idSchedule, listGuide, listGuideOld, codeTour) {
        let schedule = Parse.Object.extend('schedule');
        let guide: Array<any> = [];
        ...
        for (let i = 0; i < idSchedule.listSchedule.length; i++) {
            ...
            dataSchedule.codeSchedule = codeTour.toUpperCase() + '-' + moment(idSchedule.listSchedule[i].startDay).format('DDMMYYYY');
            let saveSchedule = await objSchedule.save(dataSchedule);
            if (listGuideOld[i].objGuide) {
                let deleteGuide = await this.deleteGuide(saveSchedule.id, listGuideOld[i].objGuide);
                if (deleteGuide) {
                    let guideRelation = saveSchedule.relation('objGuide');
                    guideRelation.add(objGuide[i]);
                    let succes = saveSchedule.save();
                    return succes;
                }
            } else {
                let guideRelation = saveSchedule.relation('objGuide');
