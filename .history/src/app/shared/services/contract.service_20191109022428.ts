...
export class ContractService {
    ...
    constructor(private http: HttpClient) {
        console.log('contractxxxxxxxxxxxxx');
    }
    async getSchedule() {
        ...
        try {
            ...
            if (data.info.idUser !== '') {
                ...
                if (result) {
                    ...
                    dataSchedule.expiryDate = await new Date(Date.now());
                ...
                if (objUser) {
                    ...
                    if (result) {
                        dataSchedule.objectId = data.info.tour;
                        dataSchedule.empty = await this.setEmpty(data.info.tour, data.quantity);
                        dataSchedule.expiryDate = moment(Date.now())
                            .add(3, 'day')
                            .format('LLL');
                        await ObjectSchedule.save(dataSchedule);
                        dataCustomer.objectId = data.idCustomer;
    ...
    async booktour(firstForm: any, secondForm: any, thirdForm: any, totalPrice: number, price: number, quantity: number) {
        ...
        try {
            ...
            if (result) {
                ...
                dataTour.expiryDate = moment(Date.now())
                    .add(3, 'day')
                    .format('LLL');
