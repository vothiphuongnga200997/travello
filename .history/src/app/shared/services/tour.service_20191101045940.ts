...
export class TourService {
    ...
    async addTour(data) {
        ...
        //     if (result) {
            for(let i = 0 ; i < data.schedule.value.listSchedule.length ; i++){
                let tourObject = tour.createWithoutData('hZG9SRsJbL');
                let schedule = Parse.Object.extend('schedule');
                for (let n = 0 ; i < data.schedule.value.listSchedule.length;) {
                    let objSchedule = new schedule();
                    let dataSchedule: any = {};
                    dataSchedule.startDay = new Date(data.schedule.value.listSchedule[n].startDay);
                    dataSchedule.endDay = new Date(data.schedule.value.listSchedule[n].endDay);
                    dataSchedule.objTour = tourObject;
                    let saveSchedule =  schedule.save(dataSchedule);
    ...
    async getGuide(objTour) {
        let guideRelation = objTour.relation('guide');
