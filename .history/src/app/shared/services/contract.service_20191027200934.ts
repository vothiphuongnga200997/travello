...
export class ContractService {
    ...
    async booktour(firstForm: any, secondForm: any, totalPrice: number, price: number, quantity: number) {
        let contract = Parse.Object.extend('contract');
        let obj = new contract();
        let dataSave: any = {};
        let dataUser: any = {};
        ...
        dataSave.objTour = dataSave.objTour = tour.createWithoutData(firstForm.idTour);
        ...
        try {
            ...
            if (result) {
                ...
                dataTour.empty = await this.setEmpty(firstForm.idTour, quantity);
                let objTour = await ObjectTour.save(dataTour);
            }
            return result;
        } catch (ex) {
    ...
    async setCustom(idTour, quantity) {
        this.booking = 0;
        this.empty = 0;
        let currentUser = Parse.User.current();
        let innerQueryUser = new Parse.Query(currentUser);
        let contract = Parse.Object.extend('contract');
        let query = new Parse.Query(contract);

        innerQueryUser.equalTo('objUser', Parse.User.current());
        query.matchesQuery('objTour', innerQueryUser);
        let result = await query.first();
        return result;
    }
}
