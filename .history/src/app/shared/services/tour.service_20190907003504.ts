...
export class TourService {
    ...
    async addGuide(data) {
        ...
        let i = 0;
        try {
            while (data.guide[i].id || data.location[i].id) {
                let classInfo = Parse.Object.extend('infoTour');
                let info = new classInfo();
                if (data.location[i].id) {
                    let classLocation = Parse.Object.extend('location');
                    var locationObject = classLocation.createWithoutData(data.location[i].id);
                } else {
                    let classGuide = Parse.Object.extend('guide');
                    var guideObject = classGuide.createWithoutData(data.guide[i].id);
                }
