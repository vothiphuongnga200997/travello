...
export class TourService {
        ...
        try {
            ...
            for (let data of result) {
                ...
                if (resultCustomer) {
                    ...
                    dataCustomer.paid = resultCustomer.get('paid') - data.get('price') - data.get('price') * data.get('discount');
            ...
            for (let destroy of scheduleDelete) {
    ...
    async deleteSchedule(scheduleDelete) {
        try {
            ...
            const query = new Parse.Query(contract);
            let repay = Parse.Object.extend('repay');
            query.equalTo('objSchedule', scheduleDelete);
            query.equalTo('status', true);
            let result = await query.find();
            for (let data of result) {
                let ObjectSchedule = new repay();
