...
export class ContractService {
    ...
    async getUser() {
        ...
        query.each(function(object, error) {}, { useMasterKey: true });
        try {
            let result = await query.fisrt();
            const res = await new Parse.Query('User').equalTo('objectId', result.id).find({ useMasterKey: true });
            console.log(res);
            console.log(result);
            return result;
        } catch (ex) {
