...
export class ContractService {
    ...
    async editContract(data) {
        ...
        dataSave.discount = 0;
        if (data.paidOfCuctomer + data.price >= 100000000) dataSave.discount = 0.1;
        if (data.paidOfCuctomer + data.price < 100000000 && data.paidOfCuctomer + data.price >= 50000000) dataSave.discount = 0.05;
        try {
            let result = await obj.save(dataSave);
            if (result) {
