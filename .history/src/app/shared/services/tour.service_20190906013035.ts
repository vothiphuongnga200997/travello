...
export class TourService {
    ...
    async addGuide(data) {
        ...
        try {
            for (let i = 0; i < data.guide.lenght; i++) {
                const id = data.guide[i].id;
                let objGuide = Parse.Object.extend('locationInfo');
                let guide = new objGuide();
                let myObject = objGuide.createWithoutData(id);
                guide.set('idLocation', myObject);
            }
        } catch (ex) {}
