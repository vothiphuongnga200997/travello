        ...
        this.subscribeGetSearchParams = this.activeRoute.queryParams.subscribe(async params => {
            ...
            if (this.dataSearchFromHomeComponet.money !== '') {
                if (this.dataSearchFromHomeComponet.money === 1) {
                    innerQueryTour.greaterThanOrEqualTo('adullPrice', 1000000);
                    innerQueryTour.lessThanOrEqualTo('adultPrice', 5000000);
                    let resultT = await innerQueryTour.find();
                    console.log('tour', resultT);
                    query.matchesQuery('objTour', innerQueryTour);
                }
                if (this.dataSearchFromHomeComponet.money === 2) {
                    innerQueryTour.greaterThanOrEqualTo('adultPrice', 5000000);
                    innerQueryTour.lessThanOrEqualTo('adultPrice', 8000000);
                    query.matchesQuery('objTour', innerQueryTour);
                }
                if (this.dataSearchFromHomeComponet.money === 3) {
                    innerQueryTour.greaterThanOrEqualTo('adulPrice', 8000000);
                    innerQueryTour.lessThanOrEqualTo('adultPrice', 10000000);
                    query.matchesQuery('objTour', innerQueryTour);
                }
                if (this.dataSearchFromHomeComponet.money === 4) {
                    query.matchesQuery('objTour', innerQueryTour);
                    innerQueryTour.greaterThan('adultPrice', 10000000);
                }
            }
            ...
            if (result) {
                ...
                for (let data of result) {
                    ...
                    let locationRelation = data.get('objTour').relation('locations');
                    ...
                    queryImg.equalTo('idTour', data.get('objTour'));
                    let resultImg = await queryImg.first();
                    this.listTour.push({
                        id: data.get('objTour').id,
                        ...
                        saleoff: data.get('objTour').get('saleoff'),
