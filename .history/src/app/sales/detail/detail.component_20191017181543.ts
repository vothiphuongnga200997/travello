...
export class DetailComponent implements OnInit {
    ...
    async onLoginFaceBookSubmit(idTour) {
        try {
            ...
            if (!users.existed()) {
                ...
                this.router.navigate(['book-tour' + id]);
            }
        } catch (error) {
    ...
    async onLoginSubmit(idTour) {
        ...
        if (this.password !== '' && this.username !== '') {
            try {
                ...
                if (login.success) {
                    this.router.navigate(['book-tour' + idTour]);
                }
            } catch (ex) {
                if (ex && ex.message) {
    ...
    ngAfterViewInit() {
        this.logInWithGoogle('google-login-button-sign-in', this.idTour);
    }
    idTour: string;
    logInWithGoogle(buttonId, idTour) {
        this.idTour = idTour;
