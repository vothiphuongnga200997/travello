...
export class BookTourComponent implements OnInit, AfterViewChecked {
    ...
    async exchange() {
        let result;
        let i = await this.http.get('https://openexchangerates.org/api/latest.json?app_id=35ecc69adc9e4459be28ae175fbc1750&base=USD');
        await i.subscribe(async res => {
            result = await res.rates['VND'];
        });
        return result;
    }
    async thanhtoan() {
        ...
        n.subscribe(async res => {
            result = await res.rates['VND'];
            let dataSendMail = {
                to: 'dyesebepn@gmail.com',
                content: `Xin chào  ` + `Mã xác nhận của bạn`,
            };
            let i = await this.contractService.pay(dataSendMail);
            i.subscribe(async data => {
                // nhận data từ server ver62 cái id nó là res
                console.log('res', data);
                window.open(data.link, 'myWindow', 'width=500,height=600');
            });
        });
        // let dataSendMail = {
        //     to: 'dyesebepn@gmail.com',
        ...
        //     // nhận data từ server ver62 cái id nó là res
        //     console.log('res', res);
        //     window.open(res.link, 'myWindow', 'width=500,height=600');
        //     this.finish = 'Bạn vui lòng kiểm tra mail nhận mã xác nhận!';
        // });
        // this.checkMail = '';
        // console.log(i);
