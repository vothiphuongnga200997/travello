...
export class BookTourComponent implements OnInit {
    ...
    transaction: boolean = true; // check khi đã giao dich
    success: boolean = false;
    notify: String; // thong bao khi hoan thanh giao dich
    ...
    async fetchAndGetUser() {
        ...
        console.log(this.paidOfCuctomer);
    }
    i: number = 0;
    ...
    async sendMail() {
        ...
        if (result) {
            ...
            this.finish = 'Hệ thống sẽ gửi mã xác nhận đến mail của bạn! Vui lòng chờ trong giây lát!';
            let dataSendMail = {
            ...
            i.subscribe(async res => {
                if (res === 'false') {
                    this.finish = 'Hệ thống không gửi được mã xác nhận bạn vui lòng kiểm tra lại!';
                } else {
    ...
    async onFourSubmit() {
        if (this.tienmat) {
            ...
            if (result) {
                ...
                this.success = true;
                this.notify =
                    'Bạn vui lòng thanh toán trước ngày' +
                    new Date(
                        moment(Date.now())
                            .add(3, 'day')
                            .format('LLL'),
