...
export class BookTourComponent implements OnInit, AfterViewChecked {
    ...
    discount: number; // giam gia cho loai khach hang
    ...
    async fetchAndGetUser() {
        ...
        if (!currentUser) {
            ...
            if (result) {
                if (result.attributes.paid >= 100000000) this.discount = 0.01;
                else {
                    if (50000000 <= result.attributes.paid && 100000000 > result.attributes.paid) this.discount = 0.005;
                    else {
                        this.discount = 1;
                    }
                }
    ...
    async sendMail() {
        let result = await this.contractService.setEmail(this.thirdForm.value.email);
        this.submitted = true;
