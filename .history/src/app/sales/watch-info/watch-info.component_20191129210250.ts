...
export class WatchInfoComponent implements OnInit {
    ...
    @ViewChild('changePasswordDialog', { static: true }) changePasswodDialog: DialogComponent;
    ...
    changePass(content) {
        if (this.infoForm.invalid) {
            ...
            this.dialogConfig = {
                title: 'Nhắc nhở',
                content: `Bạn có đồng ý thay đổi mật khẩu mới.`,
                rightBtnLabel: 'Ok',
                leftBtnLabel: 'Cancel',
                rightBtnStatus: ButtonStatusEnum.Info,
                leftBtnStatus: ButtonStatusEnum.Hint,
            };
            this.changePasswodDialog.open();
        }
        this.submitted = false;
    }
    async detail(id) {
