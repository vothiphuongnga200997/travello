...
export class HomeComponent implements OnInit {
    ...
    async selectTour() {
        ...
        const query = new Parse.Query(tour);
        query.select('nameTour', 'highlights', 'adultPrice', 'childrenPrice', 'saleoff');
        ...
        if (result.length > 0) {
            for (let i of result) {
                ...
                querySchedule.descending('startDay');
                querySchedule.equalTo('status', true);
                let objschedule = await querySchedule.first();
                if (objschedule) {
                    const queryImg = new Parse.Query(image);
                    queryImg.equalTo('idTour', i);
                    ...
                    this.tourInfo.push({
                        ...
                        name: i.attributes.nameTour,
                        highlights: i.attributes.highlights,
                        priceAdult: i.attributes.adultPrice - i.attributes.saleoff,
                        image: resultImg.attributes.image._url,
                        startDay: moment(objschedule.attributes.startDay).format('DD/MM/YYYY, hh:mm A'),
                        endDay: moment(objschedule.attributes.endDay).format('DD/MM/YYYY, hh:mm A'),
                        saleoff: i.attributes.saleoff,
                    });
                }
            }
        }
    ...
    findTour() {
        this.router.navigate([
            '/resultTour/',
            { startDay: this.startDay, endDay: this.endDay, money: this.money, location: this.location },
        ]);
    }
}
