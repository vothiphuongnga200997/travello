...
export class MenuComponent implements OnInit {
    ...
    async checkMail() {
        ...
        query.equalTo('email', this.registerForm.value.email);
        let result = await query.first();
        if (result) {
            this.noticeEmail = 'Email này đã được sử dụng.';
        } else {
            this.noticeEmail = '';
        }
    }
    async checkUser() {
        const query = new Parse.Query(Parse.User);
        query.equalTo('username', this.registerForm.value.username);
        let result = await query.first();
        if (result) {
            this.noticeUser = 'Tài khoản này đã được sử dụng.';
        } else {
            this.noticeUser = '';
        }
    }
    ...
    async onSubmit(content) {
        this.submitted = true;
        if (this.registerForm.invalid || this.noticeEmail === '' || this.noticeUser === '') {
            return;
        } else {
            try {
